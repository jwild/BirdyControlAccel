...
bool HelloWorld::init()
{
    ...
    char msgUP[10] = "^";
    CCLabelBMFont * upLabel ;
    upLabel = CCLabelBMFont::create(msgUP, "Arial.fnt");
    
    CCMenuItemLabel *upitem = CCMenuItemLabel::create(upLabel, this, menu_selector(HelloWorld::moveUp) );
    upitem->setScale(1.6);
    upitem->setPosition( ccp(winSize.width/4, winSize.height*0.1));
    
  
    buttonup = CCMenu::create(upitem, NULL);
    buttonup->setPosition(CCPointZero);
    this->addChild(buttonup);
    
   // upitem->runAction(CCScaleTo::create(0.5, 1.0));
    
    
    char msgDOWN[10] = "V";
    CCLabelBMFont * downLabel ;
    downLabel = CCLabelBMFont::create(msgDOWN, "Arial.fnt");
    
    CCMenuItemLabel *downitem = CCMenuItemLabel::create(downLabel, this, menu_selector(HelloWorld::moveDown) );
    downitem->setScale(0.1);
    downitem->setPosition( ccp(winSize.width/4, winSize.height*0.05));
    
    
    buttondown = CCMenu::create(downitem, NULL);
    buttondown->setPosition(CCPointZero);
    this->addChild(buttondown);
    
    downitem->runAction(CCScaleTo::create(0.5, 1.0));
    
    
    
    //CCTextFieldTTF

    CCTextFieldTTF * pTextField = CCTextFieldTTF::textFieldWithPlaceHolder("Enter Your Birds Name Here", "Thonburi", 20);
    pTextField->setPosition(ccp(winSize.width/2,100));
...
void HelloWorld::moveUp() {
     CCSize size = CCDirector::sharedDirector()->getWinSize();
    _birdy->setPosition(ccp(50,(size.height/2)+200));
}
